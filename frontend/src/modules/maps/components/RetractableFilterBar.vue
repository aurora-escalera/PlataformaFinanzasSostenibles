...
const filteredEntities = computed(() => {
  ...
  return props.entities.filter(e => e.name.toLowerCase().includes(entitySearch.value.toLowerCase()))
})

const filteredYears = computed(() => {
  if (!yearSearch.value) return years.value
  return years.value.filter(y => String(y).toLowerCase().includes(yearSearch.value.toLowerCase()))
})

const filteredVariables = computed(() => { 
  if (!variableSearch.value) return variablesArray.value
  const s = variableSearch.value.toLowerCase()
  return variablesArray.value.filter(v => v.label.toLowerCase().includes(s) || v.key.toLowerCase().includes(s)) 
})
